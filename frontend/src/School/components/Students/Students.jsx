...
export default function Students() {
  ...
  const fetchStudents = async () => {
    try {
      ...
      // Initialize password visibility state for all students
  ...
  const editFormik = useFormik({
    initialValues: {
      ...
      parent: "",
      parentNum: "",
      // Password fields are intentionally omitted from edit form
    },
    onSubmit: async (values) => {
      setLoading(true);
      setError(null);
  ...
  const handleEdit = (id) => {
    ...
    if (studentToEdit.studentImg) {
      setEditImageUrl(`/images/uploaded/student/${studentToEdit.studentImg}`);
    } else {
      setEditImageUrl(null);
    }
  ...
  // Fetch single student with password for display
  const fetchStudentWithPassword = async (id) => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get(`${baseApi}/student/fetch/${id}`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
              ...
              <Card key={student._id} sx={{ maxWidth: 345 }}>
                <CardActionArea>
                  <CardMedia
                    component="img"
                    height="140"
                    image={`/images/uploaded/student/${student.studentImg}`}
                    alt={student.name}
                  />
                  <CardContent>
